...
print("="*70)
print("🚀 Starting PDF RAG Chat App (Streamlit)")
print("="*70)
...
class RerankedRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(self, query: str) -> List[Document]:
        try:
            ...
            if max_score > self.confidence_threshold or score_std > 0.5:
...
def initialize_rag():
    ...
    try:
        ...
        base_retriever = VectorSearchRetriever(
            vsc_client=vsc, index=vs_index, embeddings=embeddings_model, k=15,
            text_column="content", columns=["chunk_id", "content", "source", "file_name", "page_number"]
        )
        reranker = CrossEncoder('cross-encoder/ms-marco-MiniLM-L-6-v2')
        reranked_retriever = RerankedRetriever(
            base_retriever=base_retriever, reranker=reranker, initial_k=15, final_k=10, confidence_threshold=0.3
        )
        llm = get_llm(backend=LLM_BACKEND, model_name=LLM_MODEL, temperature=TEMPERATURE, max_tokens=MAX_TOKENS)
        rag_chain = RAGChain(llm=llm, retriever=reranked_retriever, enable_mlflow=False)
...
# Initialize session state FIRST before accessing it
